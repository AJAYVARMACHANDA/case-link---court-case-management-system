...
def searchlawyers(request):
    ...
    noofcases = request.GET["noofcases"]
    qualification = request.GET["qualification"]
    type = request.GET["type"]

    for lawyer in LawyerModel.objects.all():

        lawyercases=CaseModel.objects.filter(lawyerid=lawyer.username).count()
        ...
        if qualification != "no":
            if lawyer.qualification==qualification:
                isAdd1 = "yes"
        else:
            isAdd1 = "yes"
        ...
        if int(experience) != 0:
            if int(lawyer.experience) >= int(experience):
                isAdd3 = "yes"
        else:
            isAdd3 = "yes"

        if int(noofcases) != 0:
            if int(lawyercases) >= int(noofcases):
                isAdd4 = "yes"
        else:
            isAdd4 = "yes"

        if isAdd1 == "yes" and isAdd2 == "yes" and isAdd3 == "yes" and isAdd4 == "yes":
...
def download_file(request):
    ...
    file_path=settings.BASE_DIR+"/documents/"+str(file)
    return FileResponse(open(file_path, 'rb'), as_attachment=True, filename=str(file))